...
def add_to_cart(request):
    """Add product to cart using POST parameters."""
    ...
    if not all([product_id, qty, color, size, cart_id]):
        return JsonResponse({"error": "No id, qty, color, size or cart_id"}, status=400)
    ...
    if not existing_cart_item:
        ...
        item_sub_total = existing_cart_item.sub_total
